...
const STATIC_PAGES = [
  ...
  },
  {
    ...
    content: `Contact Sozim Trading and Consultancy. We are based in Bloemfontein, South Africa. You can contact us through our website contact form, call us for a call back, or visit our campus. We are located in Bloemfontein in the Free State province of South Africa. Use our contact form, call me back service, or visit us to learn more about our accredited courses and programmes.`,
  },
  {
    url: "/campus",
    title: "Sozim Campus | Bloemfontein",
    content: `Sozim Trading and Consultancy has a campus in Bloemfontein, Free State, South Africa. Our campus provides a conducive learning environment for our students. We are based in Bloemfontein, the capital city of the Free State province. Students can access our facilities and attend classes at our campus location.`,
  },
  {
    url: "/career-pathway",
...
export const insertContentChunk = internalMutation({
  args: {
    ...
    type: v.union(v.literal("page"), v.literal("blog"), v.literal("service"), v.literal("product"), v.literal("faq"), v.literal("other")),
    indexedAt: v.number(),
    updatedAt: v.number(),
  },
  handler: async (ctx, args) => {
      ...
      for (const course of courses) {
        ...
        if (course.modules?.knowledgeModules?.length) {
          ...
          for (const m of course.modules.knowledgeModules) {
            if (m.title) content += `- ${m.title} (${m.credits || 0} credits, NQF Level ${m.nqfLevel || ""})\n`;
          }
        }
...
export const manualReindexWebsite = action({
  handler: async (ctx): Promise<{ success: boolean; pagesIndexed: number; chunksCreated: number; chunksDeleted: number }> => {
    const now = Date.now();
    const logId = await ctx.runMutation(internal.websiteIndexer.createIndexingLog, {
      status: "running",
      startedAt: now,
    });

    try {
      const result = await ctx.runMutation(internal.websiteIndexer.indexFromConvexData);
